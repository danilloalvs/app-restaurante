...
export async function getCart() {
  ...
  fromDb.forEach(fb =>
    !carts_id.includes(fb.id) ? carts_id.push(fb.id) : null
  )
...
export async function addProduct(cart, product) {
...
export async function addProductQuantity(cartProduct) {
...
export async function removeProductQuantity(cartProduct) {
  const query = "update tbCartProducts set quantity = quantity - 1 where id=?"
  await execQuery(query, [cartProduct.id])
